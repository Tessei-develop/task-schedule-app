...
export async function POST(req: NextRequest) {
  // Rate limit: 10 requests per hour per IP to protect Groq quota
  const ip = req.headers.get('x-forwarded-for')?.split(',')[0]?.trim() ?? 'unknown'
  const limit = checkRateLimit(`ai-feedback:${ip}`, 10, 60 * 60 * 1000)
  if (!limit.ok) {
    return NextResponse.json(
      { error: 'Too many requests. Please wait before requesting feedback.' },
  ...
  try {
    ...
    // Clamp periodDays to a safe range (1–90 days)
    const rawPeriod = typeof body.periodDays === 'number' ? body.periodDays : 7
    const periodDays = Math.max(1, Math.min(90, Math.floor(rawPeriod)))
    ...
    console.error('[POST /api/ai/feedback]', err)
